...
export default function AdminEventShowPage() {
    ...
    const handleApproveAttendee = async (transactionId) => {
        if (
            !window.confirm(
                "Yakin ingin menyetujui peserta ini? Tiket akan diterbitkan.",
            )
        )
            return;
        try {
            await api.post(
                `/admin/events/${event.id_event || event.id}/attendees/${transactionId}/approve`,
            );
            fetchData();
    ...
    const handleRejectAttendee = async (transactionId) => {
        if (
            !window.confirm(
                "Yakin ingin menolak peserta ini? Saldo akan dikembalikan ke wallet mereka.",
            )
        )
            return;
        try {
            await api.post(
                `/admin/events/${event.id_event || event.id}/attendees/${transactionId}/reject`,
            );
            fetchData();
    ...
    const handleSaveTier = async (e) => {
        ...
        try {
            if (editTierModalData) {
                await api.put(
                    `/admin/events/${event.id_event || event.id}/tiers/${editTierModalData.id_ticket_tier || editTierModalData.id_tier || editTierModalData.id}`,
                    tierForm,
                );
            } else {
                await api.post(
                    `/admin/events/${event.id_event || event.id}/tiers`,
                    tierForm,
                );
            }
    ...
    const handleDeleteTier = async (tierId) => {
        ...
        try {
            await api.delete(
                `/admin/events/${event.id_event || event.id}/tiers/${tierId}`,
            );
            fetchData();
        } catch (err) {
    ...
    const handleSaveTenant = async (e) => {
        ...
        try {
            if (editTenantModalData) {
                await api.put(
                    `/admin/events/${event.id_event || event.id}/tenants/${editTenantModalData.id || editTenantModalData.id_user}`,
                    tenantForm,
                );
            } else {
                await api.post(
                    `/admin/events/${event.id_event || event.id}/tenants`,
                    tenantForm,
                );
            }
    ...
    const handleDeleteTenant = async (tenantId) => {
        ...
        try {
            await api.delete(
                `/admin/events/${event.id_event || event.id}/tenants/${tenantId}`,
            );
            fetchData();
        } catch (err) {
    ...
    const handleWithdrawal = async (e) => {
        ...
        try {
            await api.post(
                `/admin/events/${event.id_event || event.id}/withdraw`,
                {
    ...
    const handleApproveWithdrawal = async (withdrawalId) => {
        if (!window.confirm("Yakin ingin menyetujui dan mencairkan penarikan tenant ini?")) return;
        try {
            await api.post(`/admin/events/${event.id_event || event.id}/withdraw/${withdrawalId}/approve`);
            fetchData();
            alert("Penarikan disetujui dan dicairkan!");
        } catch (err) {
            alert(err.response?.data?.message || "Gagal menyetujui penarikan");
        }
    };
    ...
    if (!data) {
        return (
            <div className="bg-background min-h-screen flex items-center justify-center flex-col">
                ...
                <Link
                    to="/admin/events"
                    className="mt-4 text-primary hover:underline font-label-md"
                >
    ...
    return (
        <div className="bg-background text-on-surface font-body-sm min-h-screen">
            ...
            <aside
                className="w-[240px] h-screen fixed left-0 top-0 bg-surface border-r border-outline-variant hidden md:flex flex-col py-6 z-40"
                style={{ borderRightWidth: "0.5px" }}
            >
                <div className="px-6 mb-10">
                    <h2 className="font-headline-lg text-headline-lg font-black text-primary">
                        SecureGate
                    </h2>
                    ...
                    <div className="pt-4 border-t border-outline-variant flex items-center justify-between">
                        <div className="flex items-center">
                            ...
                            <div className="ml-2 overflow-hidden">
                                ...
                                <p className="font-caption text-caption text-secondary">
                                    ID: GM-{user?.id_user || "1"}
                                </p>
                            </div>
                        </div>
                        <button
                            onClick={handleLogout}
                            className="text-primary active:opacity-70 mt-1"
                        >
            ...
            {/* Main Content Area */}
            <main className="md:ml-sidebar-width min-h-screen pb-24 md:pb-container-padding">
                {/* Hero Banner Section */}
                <section className="max-w-[1200px] mx-auto px-0 md:px-container-padding md:pt-container-padding">
                    ...
                    {/* Event Basic Info */}
                    <div className="px-container-padding md:px-0 mt-6 flex flex-col md:flex-row md:justify-between md:items-end gap-4">
                        <div>
                            <span className="text-tertiary font-label-md text-label-md bg-tertiary-fixed px-3 py-1 rounded-full">
                                {event.category}
                            </span>
                            <h2 className="font-headline-xl text-headline-xl md:hidden mt-2 text-on-surface">
                                {event.title}
                            </h2>
                        ...
                        <div className="flex gap-3">
                            ...
                            <button
                                ...
                                className="flex items-center gap-2 px-6 py-2 bg-primary text-on-primary rounded-lg font-bold active:scale-95 transition-transform"
                            >
                                <span className="material-symbols-outlined text-[20px]">
                                    {event.status === "active"
                                        ? "edit"
                ...
                {/* Tabs Navigation */}
                <section className="max-w-[1200px] mx-auto mt-10 px-container-padding">
                    ...
                    {activeTab === "tiket" && (
                        <div className="mt-8 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 animate-fade-in">
                            {event.ticket_tiers?.map((tier) => {
                                const sold =
                                    tier.capacity - tier.remaining_seats;
                                const pct =
                                    tier.capacity > 0
                                        ? Math.min(
                                              100,
                                              Math.round(
                                                  (sold / tier.capacity) * 100,
                                              ),
                                          )
                                        : 0;
                                const isSoldOut =
                                    pct >= 100 && !tier.is_unlimited;
                                ...
                                return (
                                    <div
                                        ...
                                        className={`bg-surface-container-lowest border-[0.5px] border-outline-variant rounded-xl p-6 transition-all hover:border-primary/40 group ${isSoldOut ? "border-dashed border-2 opacity-75" : ""}`}
                                    >
                                        ...
                                        <div className="flex items-baseline gap-1 mb-6">
                                            <span
                                                className={`font-body-sm text-body-sm font-bold ${isSoldOut ? "text-secondary" : "text-primary"}`}
                                            >
                                                IDR
                                            </span>
                                            <span
                                                className={`font-headline-lg text-headline-lg font-black ${isSoldOut ? "text-secondary" : "text-primary"}`}
                                            >
                                                {formatRpNum(tier.price)}
                                            </span>
                                        </div>
                                        <div className="space-y-3">
                                            ...
                                            {!tier.is_unlimited && (
                                                <div
                                                    className={`w-full h-2 rounded-full overflow-hidden ${isSoldOut ? "bg-error-container" : "bg-surface-container-high"}`}
                                                >
                                                    <div
                                                        className={`h-full rounded-full ${isSoldOut ? "bg-error w-full" : "bg-primary"}`}
                                                        style={{
                                                            width: `${pct}%`,
                                                        }}
                                                    ></div>
                                            ...
                                            <div className="flex flex-wrap gap-2 mt-4">
                                                {isSoldOut ? (
                                                    <span className="bg-error-container text-on-error-container px-3 py-1 rounded-full font-caption text-caption uppercase font-bold">
                                                        Sold Out
                                                    </span>
                                                ) : tier.is_unlimited ? (
                                                    <span className="bg-tertiary-fixed text-on-tertiary-fixed-variant px-3 py-1 rounded-full font-caption text-caption">
                                                        Available
                                                    </span>
                                                ) : (
                                                    <span className="bg-primary-fixed text-on-primary-fixed-variant px-3 py-1 rounded-full font-caption text-caption">
                                                        Terbatas
                                                    </span>
                                                )}
                                            </div>
                        ...
                        <div className="mt-8 animate-fade-in">
                            ...
                            <div className="flex flex-col md:flex-row gap-stack-md mb-stack-lg items-center">
                                <div className="relative flex-1 w-full">
                                    ...
                                    <input
                                        className="w-full pl-10 pr-4 py-2.5 bg-surface-container-low border-[0.5px] border-outline-variant rounded-lg focus:outline-none focus:border-primary transition-colors font-body-sm"
                                ...
                                <div className="overflow-x-auto">
                                    <table className="w-full text-left border-collapse">
                                        <thead className="bg-surface-container-low border-b-[0.5px] border-outline-variant">
                                            <tr>
                                                <th className="px-6 py-4 font-label-md text-secondary uppercase tracking-wider">
                                                    Peserta
                                                </th>
                                                <th className="px-6 py-4 font-label-md text-secondary uppercase tracking-wider">
                                                    Email
                                                ...
                                                <th className="px-6 py-4 font-label-md text-secondary uppercase tracking-wider text-center">
                                                    Status
                                                </th>
                                                <th className="px-6 py-4 font-label-md text-secondary uppercase tracking-wider text-right">
                                                    Aksi
                                                </th>
                                            </tr>
                                        </thead>
                                            ...
                                            {ticket_buyers?.length === 0 ? (
                                                ...
                                                ticket_buyers?.map((tb) => (
                                                    ...
                                                    >
                                                        ...
                                                        <td className="px-6 py-4">
                                                            <span className="px-2 py-0.5 rounded-full text-caption bg-surface-container-high text-secondary font-bold">
                                                                {
                                                                    tb
                                                                        .ticket_tier
                                                                        ?.tier_name
                                                                    ...
                                                                    <>
                                                                        <button
                                                                            ...
                                                                            onClick={async () => {
                                                                                try {
                                                                                    await api.post(
                                                                                        `/admin/events/${event.id_event || event.id}/tickets/${tb.id}/toggle-checkin`,
                                                                                    );
                                                                                    fetchData();
                                                                                } catch (err) {
                                                                                    alert(
                                                                                        "Gagal mengubah status check-in",
                                                                                    );
                                                                                }
                                                                            }}
                                                                        >
                        ...
                        <div className="mt-8 animate-fade-in">
                            ...
                            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
                                <div className="relative w-full md:w-80">
                                    ...
                                    <input
                                        className="w-full pl-10 pr-4 py-2.5 bg-surface-container-low border-[0.5px] border-outline-variant rounded-xl focus:outline-none focus:border-primary transition-colors text-body-sm"
                                                ...
                                                tenants?.map((t) => (
                                                    ...
                                                    >
                                                        ...
                                                        <td className="px-6 py-4 font-medium">
                                                            {formatRp(t.total_sales)}
                                                        </td>
                                                        <td className="px-6 py-4 text-on-surface-variant">
                                                            {formatRp(t.tenant_cut)}
                                                        </td>
                                                        <td className="px-6 py-4">
                                                            <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-caption font-medium ${t.wd_status === 'Selesai' ? 'bg-green-100 text-green-800' : (t.wd_status === 'Diproses' ? 'bg-yellow-100 text-yellow-800' : 'bg-secondary-container text-secondary')}`}>
                                                                {t.wd_status}
                                                            </span>
                                                        </td>
                            ...
                            <div className="bg-primary-fixed/30 border-[0.5px] border-primary/20 rounded-xl p-6 flex items-center justify-between">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        <p className="text-secondary text-caption font-medium uppercase tracking-tight">
                                            Estimasi Pendapatan Penyelenggara
                                        </p>
                                        <h3 className="font-headline-md text-on-primary-fixed-variant">
                                            Total Tenant Cut masuk ke pendapatan
                                            event:{" "}
                                            <span className="font-bold">
                                                {formatRp(stats?.tenant_cut)}
                                            </span>
                                        </h3>
                                    </div>
                                </div>
                                <button
                                    className="hidden md:block text-primary font-label-md hover:underline decoration-2 underline-offset-4"
                        ...
                        <div className="mt-8 animate-fade-in space-y-6">
                            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                                ...
                                <div className="lg:col-span-2 bg-white rounded-xl border-[0.5px] border-outline-variant p-6 h-fit">
                                    ...
                                    <div className="space-y-4">
                                        ...
                                        <div className="flex justify-between items-center py-2 border-b-[0.5px] border-outline-variant border-dashed">
                                            ...
                                            <span className="text-body-sm font-medium text-tertiary">
                                                +{formatRp(stats?.tenant_cut)}
                                            </span>
                                        </div>
                                ...
                                <div className="bg-surface-container-low rounded-xl border-[0.5px] border-outline-variant p-6 flex flex-col justify-between">
                                    <div>
                                        <p className="text-caption text-secondary uppercase tracking-wider font-bold mb-1">
                                            Saldo Tersedia
                                        </p>
                                        <h4 className="text-headline-lg font-black text-on-surface mb-2">
                                            {formatRp(
                                                stats?.available_to_withdraw,
                                            )}
                                        </h4>
                                        ...
                                        <table className="w-full text-left border-collapse">
                                            <thead>
                                                <tr className="bg-surface-container-low">
                                                    <th className="px-6 py-3 text-caption font-bold text-secondary uppercase">
                                                        Tanggal Pengajuan
                                                    </th>
                                                    <th className="px-6 py-3 text-caption font-bold text-secondary uppercase">
                                                        ID Transaksi
                                                    ...
                                                    <th className="px-6 py-3 text-caption font-bold text-secondary uppercase text-right">
                                                        Aksi
                                                    </th>
                                                </tr>
                                            </thead>
                                            <tbody className="divide-y-[0.5px] divide-outline-variant">
                                                {data.event_withdrawals
                                                    ?.length > 0 ? (
                                                    data.event_withdrawals.map(
                                                        (wd) => (
                                                            <tr
                                                                key={wd.id}
                                                                className="hover:bg-surface-container-lowest transition-colors"
                                                            >
                                                                ...
                                                                <td className="px-6 py-4">
                                                                    <span
                                                                        className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-caption font-bold ${wd.status === "success" ? "bg-green-100 text-green-800" : "bg-yellow-100 text-yellow-800"}`}
                                                                    >
                                                                        {wd.status ===
                                                                        "success"
                                                                            ? "Berhasil"
                            ...
                            {pending_withdrawals?.length > 0 && (
                                <div className="bg-orange-50 border-[0.5px] border-orange-200 rounded-xl p-6">
                                    <h3 className="font-bold text-orange-800 font-headline-md mb-4 flex items-center">
                                        <span className="material-symbols-outlined mr-2">
                                            notifications_active
                                        </span>{" "}
                                        Permintaan Penarikan Tenant
                                    </h3>
                                    <div className="space-y-4">
                                        {pending_withdrawals.map((pw) => (
                                            <div
                                                key={pw.id}
                                                className="bg-white p-4 rounded-lg border-[0.5px] border-orange-100 flex flex-col md:flex-row md:items-center justify-between gap-4"
                                            >
                                                <div>
                                                ...
                                                <button 
                                                    className="px-4 py-2 bg-green-500 hover:bg-green-600 text-white rounded-lg font-label-md transition-colors shadow-sm"
                                                    onClick={() => handleApproveWithdrawal(pw.id)}
                                                >
                                                    Approve & Cairkan
                                                </button>
                                            </div>
            ...
            <nav className="fixed bottom-0 w-full z-50 md:hidden bg-surface border-t-[0.5px] border-outline-variant flex justify-around items-center h-16 pb-safe">
                <Link
                    to="/admin/dashboard"
                    className="flex flex-col items-center text-secondary active:bg-surface-container-low px-4 py-1 transition-colors"
                >
                ...
                <Link
                    to="/admin/events"
                    className="flex flex-col items-center text-primary font-bold active:bg-surface-container-low px-4 py-1 transition-colors"
                >
                    <span className="material-symbols-outlined">
                        confirmation_number
                    </span>
                        ...
                        <div className="p-6">
                            <div className="bg-error-container/20 p-4 rounded-lg mb-6 flex gap-3">
                                ...
                                <p className="text-error font-body-sm">
                                    Tindakan ini tidak dapat dibatalkan. Dana
                                    (93%) akan dikembalikan ke metode pembayaran
                                    asal pembeli.
                                </p>
                            </div>
                            <div className="space-y-4">
                                <div>
                                    <p className="text-secondary font-label-md mb-1">
                                        Nama Peserta
                                    </p>
                                    <p className="font-bold text-on-surface">
                                        {refundModalData.user?.full_name}
                                    </p>
                                </div>
                                <div>
                                    <p className="text-secondary font-label-md mb-1">
                                ...
                                <div>
                                    ...
                                    <p className="text-headline-lg font-black text-primary">
                                        {formatRp(refundModalData.gross_amount)}
                                    </p>
                                </div>
                            </div>
                        </div>
                        <div className="px-6 py-5 bg-surface-container-low flex flex-col md:flex-row-reverse gap-3">
                            <button
                                ...
                                onClick={async () => {
                                    ...
                                    try {
                                        await api.post(
                                            `/admin/events/${event.id_event || event.id}/tickets/${refundModalData.id}/refund`,
                                        );
                                        fetchData();
                            ...
                            <button
                                className="flex-1 py-2.5 bg-transparent border-[0.5px] border-outline text-secondary font-bold rounded-lg hover:bg-surface-container-high transition-all"
                                onClick={() => setRefundModalData(null)}
                            >
                                Batal
                            </button>
                    ...
                    <div className="bg-white w-full max-w-md rounded-xl border-[0.5px] border-outline-variant overflow-hidden">
                        ...
                        <form onSubmit={handleSaveTier}>
                            <div className="p-6 space-y-4">
                                <div>
                                    <label className="block text-secondary font-label-md mb-1">
                                        Nama Tier
                                    </label>
                                    <input
                                        type="text"
                                ...
                                <div>
                                    ...
                                    {editTierModalData && (
                                        <p className="text-secondary text-caption mt-1">
                                            Kuota tidak boleh kurang dari tiket
                                            yang sudah terjual.
                                        </p>
                                    )}
                                </div>
                            ...
                            <div className="px-6 py-5 bg-surface-container-low flex gap-3">
                                ...
                                <button
                                    type="submit"
                                    className="flex-1 py-2.5 bg-primary text-white font-bold rounded-lg hover:opacity-90 transition-all active:scale-95"
                                >
                                    Simpan Tier
                                </button>
                            </div>
                            ...
                            <div className="space-y-4">
                                <div>
                                    ...
                                    <input
                                        ...
                                        required
                                        min="10000"
                                        max={stats?.available_to_withdraw || 0}
                                        value={wdAmount}
                                        onChange={(e) =>
                                    ...
                                    <p className="text-[11px] text-secondary mt-1">
                                        Maksimal: Rp{" "}
                                        {formatRp(stats?.available_to_withdraw)}
                                    </p>
                                </div>
                                <div>
                                    <label className="block font-label-md text-on-surface-variant mb-1">
                                ...
                                <div className="pt-4 flex gap-3">
                                    ...
                                    <button
                                        type="button"
                                        className="w-full bg-surface-container-low text-on-surface py-3 rounded-lg font-bold hover:bg-surface-container-high transition-colors"
                                        onClick={() =>
                                            setWithdrawalModalOpen(false)
                                        }
                                    >
                                    ...
                                    <button
                                        disabled={wdLoading}
                                        type="submit"
                                        className="w-full bg-primary text-white py-3 rounded-lg font-bold hover:opacity-90 transition-all shadow-md flex items-center justify-center"
                                    >
                                        {" "}
                                        {wdLoading ? (
