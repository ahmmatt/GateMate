...
export default function TenantDashboardPage() {
  ...
  const handleLogout = async () => {
    ...
    logout(); navigate('/organizer/login');
  };
  ...
  const handleAddMenu = async (e) => {
    ...
    try {
      const res = await api.post('/tenant/menus', {
  ...
  const handleWithdraw = async (e) => {
    ...
    try {
      const res = await api.post('/tenant/withdraw', {
  ...
  const generateQr = () => {
    ...
    setQrPayload({ id: user?.id || user?.id_user, amount: grandTotal });
  };
