...
<div class="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
            ...
            <div class="lg:col-span-7 flex flex-col gap-6">
                ...
                <div class="flex flex-col gap-1">
                    <h1 class="font-headline-lg text-headline-lg md:text-headline-lg font-bold tracking-tight">{{ $transaction->event->title ?? 'Unknown Event' }}</h1>
                    <div class="flex items-center gap-4 text-secondary mt-1">
                        <div class="flex items-center gap-1">
                            <span class="material-symbols-outlined text-sm">calendar_today</span>
                            <span class="font-body-md text-body-md">
                                {{ \Carbon\Carbon::parse($transaction->event->start_date)->translatedFormat('d M Y') }}
                            </span>
                        </div>
                        <div class="flex items-center gap-1">
                            <span class="material-symbols-outlined text-sm">location_on</span>
                            <span class="font-body-md text-body-md">
                                @if ($transaction->event->location_type === 'online')
                                    Virtual Meeting (Online)
                                @else
                                    {{ !empty($transaction->event->venue_name) ? $transaction->event->venue_name . ', ' : '' }}
                ...
                <div class="bg-white border border-[#EBEBEB] rounded-[14px] p-8 flex flex-col items-center gap-6 shadow-sm relative">
                    ...
                    <div class="w-64 h-64 bg-white border-2 border-on-surface p-4 flex items-center justify-center relative">
                        <div class="w-full h-full relative flex items-center justify-center">
                            ...
                            <img src="https://api.qrserver.com/v1/create-qr-code/?size=250x250&data={{ urlencode($transaction->order_id) }}&color=000000&bgcolor=ffffff" 
                                 alt="QR Code" 
                    ...
                    <div class="text-center flex flex-col gap-2">
                        ...
                        <p class="font-headline-sm text-headline-sm font-bold tracking-wider">{{ $transaction->order_id }}</p>
                    </div>
                    ...
                    <div class="w-full border-t border-[#EBEBEB] pt-6 grid grid-cols-2 gap-y-5 gap-x-4">
                        <div class="flex flex-col gap-1">
                            ...
                            <p class="font-body-md text-body-md font-medium">{{ $transaction->user->full_name ?? 'Peserta' }}</p>
                        </div>
                        <div class="flex flex-col gap-1 text-right">
                            ...
                            <div class="flex justify-end">
                                <span class="bg-[#FFF0EE] text-[#B83020] px-3 py-0.5 rounded-[10px] font-label-md text-label-md">
                                    {{ $transaction->ticketTier->tier_name ?? '-' }}
                                </span>
                            </div>
                        </div>
                        <div class="flex flex-col gap-1">
                            <p class="font-caption text-caption text-secondary">Start Time</p>
                            <p class="font-body-md text-body-md font-medium">{{ \Carbon\Carbon::parse($transaction->event->start_time)->format('H:i') }}</p>
                        </div>
                        <div class="flex flex-col gap-1 text-right">
                            <p class="font-caption text-caption text-secondary">End Time</p>
                            <p class="font-body-md text-body-md font-medium">{{ \Carbon\Carbon::parse($transaction->event->end_time)->format('H:i') }}</p>
                        </div>
                        <div class="flex flex-col gap-1">
            ...
            <div class="lg:col-span-5 flex flex-col gap-6">
                ...
                <div class="bg-[#FFF0EE] border border-outline-variant rounded-[14px] p-card-padding flex flex-col gap-4">
                    ...
                    @endphp
                    <button class="w-full border py-[10px] px-[22px] rounded-full font-body-md font-medium transition-all {{ $hasVibeBio ? 'border-primary text-primary hover:bg-primary hover:text-white' : 'border-outline-variant text-outline bg-surface-container-lowest cursor-not-allowed' }}" 
                            @if($hasVibeBio) onclick="openMatchmakingModal()" @else disabled title="Silakan isi Vibe Bio terlebih dahulu" @endif>
                        Mulai Pencocokan AI
                    </button>
                </div>
    ...
    <!-- ========================================== -->
    <div class="fixed inset-0 z-50 flex items-center justify-center p-container-padding transition-opacity duration-300 {{ $errors->has('vibe_bio') ? '' : 'hidden' }}" id="vibe-modal">
        <!-- Backdrop -->
        ...
        <div class="relative w-full max-w-[480px] bg-white rounded-[14px] shadow-2xl overflow-hidden border-[0.5px] border-[#EBEBEB] animate-in fade-in zoom-in duration-300">
            <form action="{{ route('ticket.vibe', $transaction->id) }}" method="POST">
                @csrf
                <div class="p-8 flex flex-col gap-6">
                    @if($errors->has('vibe_bio'))
                        <div class="bg-red-50 text-red-600 p-3 rounded-lg text-sm mb-4">
                            {{ $errors->first('vibe_bio') }}
                        </div>
                    @endif
                    <!-- Header -->
                    <div class="flex flex-col gap-2">
        ...
        <main class="flex-grow">
            ...
            <section class="max-w-[1280px] mx-auto px-container-padding -mt-8 mb-16">
                <div class="grid grid-cols-1 md:grid-cols-3 gap-gap-default">
                    @if(isset($otherAttendees) && $otherAttendees->isNotEmpty())
                        @foreach($otherAttendees as $index => $match)
                            @php
                            ...
                            <!-- Card -->
                            <div class="bg-white {{ $isBestMatch ? 'border-2 border-primary' : 'border-[0.5px] border-outline-variant' }} p-6 rounded-[14px] shadow-sm flex flex-col items-center relative transform hover:-translate-y-1 transition-all duration-300">
                                @if($isBestMatch)
                                ...
                                @endif
                                    <img alt="{{ $attName }}" src="{{ $attPic }}" class="w-full h-full object-cover">
                                </div>
                                <h3 class="font-headline-sm text-headline-sm text-on-surface mb-1">{{ $attName }}</h3>
                                ...
                                <div class="flex flex-col items-center gap-2 mb-4">
                                    @if($attIg)
                                    <a href="https://instagram.com/{{ $attIg }}" target="_blank" class="flex items-center gap-1.5 text-primary hover:opacity-80 transition-opacity">
                                        <span class="material-symbols-outlined text-[18px]">camera</span>
                                        <span class="font-label-md text-label-md">{{ '@' . $attIg }}</span>
                                    </a>
                                    @endif
                                </div>
                                <button class="mt-auto w-full py-2.5 px-6 border border-primary text-primary font-medium rounded-full hover:bg-[#d63b27] hover:text-white transition-all flex items-center justify-center gap-2" onclick="openChatModal('{{ addslashes($attName) }}', '{{ $attPic }}', '{{ addslashes(preg_replace('/\s+/', ' ', $vibeBio)) }}')">
        ...
        <header class="bg-surface border-b-[0.5px] border-outline-variant flex justify-between items-center w-full px-container-padding h-16 sticky top-0 z-50">
            ...
            <div class="flex items-center gap-4">
                ...
                <div class="h-8 w-8 rounded-full overflow-hidden border border-outline-variant">
                    ...
                    @endphp
                    <img alt="User profile" src="{{ $userProfilePath }}"/>
                </div>
            </div>
        </header>
