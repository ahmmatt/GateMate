...
<nav class="hidden md:flex gap-6 items-center">
    ...
    @else
        @if(Auth::user()->role === 'user')
            <a class="font-body-md text-body-md text-secondary hover:text-primary transition-colors" href="{{ route('discover') }}">Explore</a>
            <a class="font-body-md text-body-md text-secondary hover:text-primary transition-colors" href="{{ route('my-tickets') }}">My Tickets</a>
            <a class="font-body-md text-body-md text-secondary hover:text-primary transition-colors" href="{{ route('wallet.index') }}">Wallet</a>
        @elseif(Auth::user()->role === 'admin')
            <a class="font-body-md text-body-md text-secondary hover:text-primary transition-colors" href="{{ route('admin.dashboard') }}">Dashboard Event</a>
            <a class="font-body-md text-body-md text-secondary hover:text-primary transition-colors" href="{{ route('admin.scanner') }}">Scanner</a>
        @elseif(Auth::user()->role === 'superadmin')
            <a class="font-body-md text-body-md text-secondary hover:text-primary transition-colors" href="{{ route('superadmin.dashboard') }}">Superadmin Panel</a>
        @elseif(Auth::user()->role === 'tenant')
            <a class="font-body-md text-body-md text-secondary hover:text-primary transition-colors" href="{{ route('tenant.dashboard') }}">Tenant Dashboard</a>
        @endif
    @endguest
</nav>
</div>
<div class="flex items-center gap-4">
        ...
        <div class="flex items-center gap-4">
            ...
            <div class="relative group">
                <button class="w-10 h-10 rounded-full overflow-hidden border-2 border-outline-variant hover:border-primary transition-colors">
                    ...
                    @endphp
                    @if (!empty($navPic))
                        <img src="{{ asset('Media/uploads/' . $navPic) }}" class="w-full h-full object-cover" alt="User Profile">
                    @else
                        <div class="w-full h-full bg-primary text-white flex items-center justify-center font-bold">{{ $navInitial }}</div>
                    @endif
                </button>
                <div class="absolute right-0 mt-2 w-48 bg-white border border-outline-variant rounded-lg shadow-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all">
                    <a href="{{ url('/settings') }}" class="block px-4 py-2 text-on-surface hover:bg-surface-container-low rounded-t-lg">Settings</a>
    ...
    @else
        <div class="flex items-center gap-2">
            <a href="{{ route('signin') }}" class="font-body-md text-body-md font-bold text-primary">Sign In</a>
            <a href="{{ route('signup') }}" class="px-4 py-2 bg-primary text-white rounded-full font-label-md text-label-md font-bold hover:bg-surface-tint transition-all">Sign Up</a>
        </div>
    @endauth
...
<nav class="md:hidden fixed bottom-0 left-0 w-full z-50 bg-surface/80 dark:bg-surface-container-highest/80 backdrop-blur-md border-t border-outline-variant/30 flex justify-around items-center px-2 py-3 pb-safe">
    @guest
        <a class="flex flex-col items-center justify-center text-secondary dark:text-secondary-fixed-dim font-label-md text-label-md active:bg-surface-container-high transition-all" href="{{ route('landing') }}">
            <span class="material-symbols-outlined">explore</span>
            <span class="">Explore</span>
        </a>
    @else
        @if(Auth::user()->role === 'user')
            <a class="flex flex-col items-center justify-center text-primary dark:text-primary-fixed-dim bg-primary-fixed/20 rounded-full px-3 py-1 font-label-md text-label-md active:bg-surface-container-high transition-all" href="{{ route('discover') }}">
                <span class="material-symbols-outlined" style="font-variation-settings: 'FILL' 1;">explore</span>
                <span class="">Discover</span>
            </a>
            <a class="flex flex-col items-center justify-center text-secondary dark:text-secondary-fixed-dim font-label-md text-label-md active:bg-surface-container-high transition-all" href="{{ route('my-tickets') }}">
                <span class="material-symbols-outlined">confirmation_number</span>
                <span class="">My Tickets</span>
            </a>
            <a class="flex flex-col items-center justify-center text-secondary dark:text-secondary-fixed-dim font-label-md text-label-md active:bg-surface-container-high transition-all" href="{{ route('wallet.index') }}">
                <span class="material-symbols-outlined">account_balance_wallet</span>
                <span class="">Wallet</span>
            </a>
            <a class="flex flex-col items-center justify-center text-secondary dark:text-secondary-fixed-dim font-label-md text-label-md active:bg-surface-container-high transition-all" href="{{ url('/settings') }}">
                <span class="material-symbols-outlined">person</span>
                <span class="">Profile</span>
            </a>
        @elseif(Auth::user()->role === 'admin')
            <a class="flex flex-col items-center justify-center text-secondary dark:text-secondary-fixed-dim font-label-md text-label-md active:bg-surface-container-high transition-all" href="{{ route('admin.dashboard') }}">
                <span class="material-symbols-outlined">table_chart</span>
                <span class="">Dashboard</span>
            </a>
            <a class="flex flex-col items-center justify-center text-secondary dark:text-secondary-fixed-dim font-label-md text-label-md active:bg-surface-container-high transition-all" href="{{ route('admin.scanner') }}">
                <span class="material-symbols-outlined">qr_code_scanner</span>
                <span class="">Scanner</span>
            </a>
        @elseif(Auth::user()->role === 'superadmin')
            <a class="flex flex-col items-center justify-center text-secondary dark:text-secondary-fixed-dim font-label-md text-label-md active:bg-surface-container-high transition-all" href="{{ route('superadmin.dashboard') }}">
                <span class="material-symbols-outlined">admin_panel_settings</span>
                <span class="">Superadmin</span>
            </a>
        @elseif(Auth::user()->role === 'tenant')
            <a class="flex flex-col items-center justify-center text-secondary dark:text-secondary-fixed-dim font-label-md text-label-md active:bg-surface-container-high transition-all" href="{{ route('tenant.dashboard') }}">
