...
<aside class="fixed left-0 top-0 h-full w-[240px] z-30 bg-surface-container-lowest border-r-[0.5px] border-outline-variant hidden lg:flex flex-col py-stack-lg">
    ...
    <nav class="flex-1">
        <ul class="space-y-1">
            <li>
                <a class="flex items-center px-6 py-3 text-on-surface-variant hover:bg-secondary-container transition-colors font-label-md text-label-md" href="<?php echo e(route('admin.dashboard')); ?>">
                    <span class="material-symbols-outlined mr-3">dashboard</span> Dashboard
                </a>
            </li>
            <li>
                <a class="flex items-center px-6 py-3 text-on-surface-variant hover:bg-secondary-container transition-colors font-label-md text-label-md" href="<?php echo e(route('admin.events.index')); ?>">
                    <span class="material-symbols-outlined mr-3">event</span> Events Saya
                </a>
            </li>
            <li>
                <a class="flex items-center px-6 py-3 sidebar-active font-label-md text-label-md" href="<?php echo e(route('admin.finance')); ?>">
                    <span class="material-symbols-outlined mr-3">payments</span> Keuangan
                </a>
            </li>
            <li>
                <a class="flex items-center px-6 py-3 text-on-surface-variant hover:bg-secondary-container transition-colors font-label-md text-label-md" href="<?php echo e(route('admin.scanner')); ?>">
    ...
    <div class="px-6 mt-auto">
        <div class="flex items-center gap-3 p-3 bg-surface-container rounded-lg">
            <div class="w-8 h-8 rounded-full bg-primary-fixed flex items-center justify-center text-primary font-bold">
                <?php echo e(strtoupper(substr(auth()->user()->full_name ?? 'O', 0, 1))); ?>

            </div>
            <div class="overflow-hidden">
                <p class="font-label-md text-label-md truncate font-bold"><?php echo e(auth()->user()->full_name ?? 'Organizer Profile'); ?></p>
    ...
    <div class="max-w-[1200px] mx-auto p-page-padding space-y-stack-lg">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-gutter">
            ...
            <div class="bg-surface-container-lowest p-6 rounded-[14px] flat-border flex flex-col justify-between h-[160px]">
                ...
                <div>
                    <h2 class="font-h1 text-h1 text-on-surface font-black">Rp <?php echo e(number_format($netIncomeTotal, 0, ',', '.')); ?></h2>
            ...
            <div class="bg-surface-container-lowest p-6 rounded-[14px] flat-border flex flex-col justify-between h-[160px]">
                ...
                <div>
                    <h2 class="font-h1 text-h1 text-on-surface font-black">Rp <?php echo e(number_format($sisaBisaDitarikTotal, 0, ',', '.')); ?></h2>
            ...
            <div class="bg-surface-container-lowest p-6 rounded-[14px] flat-border flex flex-col justify-between h-[160px]">
                ...
                <div>
                    <h2 class="font-h1 text-h1 text-on-surface font-black">Rp <?php echo e(number_format($pendingWithdrawals, 0, ',', '.')); ?></h2>
        ...
        <div class="bg-surface-container-lowest rounded-[14px] flat-border overflow-hidden">
                ...
                <table class="w-full text-left border-collapse">
                    ...
                    <tbody class="divide-y-[0.5px] divide-outline-variant">
                        ...
                        <tr class="hover:bg-surface-container transition-colors group">
                            <td class="px-6 py-4 font-body-sm text-body-sm"><?php echo e($trx->created_at->format('d M Y, H:i')); ?></td>
                            <td class="px-6 py-4">
                                <p class="font-body-sm text-body-sm font-medium">
                                    <?php if($trx->type === 'tenant_revenue'): ?>
                                        Bagi Hasil Tenant (<?php echo e($trx->meta['tenant_name'] ?? 'Tenant'); ?>)
                                    <?php elseif($trx->type === 'withdrawal'): ?>
                                        Penarikan Dana ke <?php echo e($trx->meta['bank_name'] ?? 'Bank'); ?> <?php echo e($trx->meta['account_number'] ?? ''); ?>

                                    <?php else: ?>
                                        <?php echo e(ucfirst(str_replace('_', ' ', $trx->type))); ?>

                                    <?php endif; ?>
                                </p>
                                <p class="text-[11px] text-on-surface-variant">Trx #<?php echo e($trx->order_id); ?></p>
                            </td>
                            <td class="px-6 py-4">
                                <?php if($trx->type === 'tenant_revenue'): ?>
                            ...
                            <td class="px-6 py-4">
                                <?php if(in_array($trx->status, ['success', 'settlement'])): ?>
                                ...
                                <?php elseif(in_array($trx->status, ['pending', 'pending_superadmin'])): ?>
                            ...
                            <td class="px-6 py-4 text-right font-medium <?php echo e($trx->type === 'withdrawal' ? 'text-on-surface' : 'text-emerald-600'); ?>">
                                <?php echo e($trx->type === 'withdrawal' ? '-' : '+'); ?> Rp <?php echo e(number_format($trx->amount, 0, ',', '.')); ?>
            ...
            <div class="p-6 border-t-[0.5px] border-outline-variant flex items-center justify-between">
                <p class="text-caption text-on-surface-variant">Menampilkan <?php echo e($transactions->count()); ?> transaksi</p>
...
<nav class="fixed bottom-0 w-full z-50 lg:hidden bg-surface-container-lowest border-t-[0.5px] border-outline-variant flex justify-around items-center h-16">
    <a class="flex flex-col items-center text-on-surface-variant active:bg-secondary-container transition-all scale-95 duration-100" href="<?php echo e(route('admin.dashboard')); ?>">
        <span class="material-symbols-outlined">home</span>
        <span class="font-label-md text-[10px]">Home</span>
    </a>
    <a class="flex flex-col items-center text-primary active:bg-secondary-container transition-all scale-95 duration-100" href="<?php echo e(route('admin.finance')); ?>">
        <span class="material-symbols-outlined" style="font-variation-settings: 'FILL' 1;">payments</span>
        <span class="font-label-md text-[10px]">Keuangan</span>
    </a>
    <a class="flex flex-col items-center text-on-surface-variant active:bg-secondary-container transition-all scale-95 duration-100" href="<?php echo e(route('admin.scanner')); ?>">
...
<div class="fixed inset-0 z-[100] hidden flex items-center justify-center bg-on-background/40 backdrop-blur-[2px] p-4" id="withdrawModal">
    <div class="bg-surface-container-lowest w-full max-w-md rounded-[20px] shadow-2xl p-6 sm:p-8 animate-in zoom-in-95 duration-200">
        ...
        <form action="<?php echo e(route('admin.finance.withdraw')); ?>" method="POST">
            <?php echo csrf_field(); ?>
            <div class="space-y-4">
                <div>
                    <label class="block font-label-md text-on-surface-variant mb-1">Nominal (Rp)</label>
                    <input type="number" name="amount" required min="10000" max="<?php echo e($sisaBisaDitarikTotal); ?>"
                    ...
                    <p class="text-[11px] text-secondary mt-1">Maksimal: Rp <?php echo e(number_format($sisaBisaDitarikTotal, 0, ',', '.')); ?></p>
                </div>
                <div>
...
</body></html><?php /**PATH D:\laragon\www\JVC26\gatemate\resources\views/admin/finance.blade.php ENDPATH**/ ?>