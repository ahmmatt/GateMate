...
<aside class="w-sidebar-width h-screen fixed left-0 top-0 hidden md:flex flex-col py-page-padding bg-surface border-r-[0.5px] border-outline-variant z-50">
    ...
    <nav class="flex-grow flex flex-col">
        <!-- Dashboard Tab -->
        <a class="flex items-center px-6 py-3 space-x-3 text-secondary hover:bg-surface-container-low transition-colors cursor-pointer active:opacity-80" href="<?php echo e(route('admin.dashboard')); ?>">
            <span class="material-symbols-outlined">dashboard</span>
            <span class="font-body-sm text-body-sm">Dashboard</span>
        </a>
        <!-- Event Saya Tab (ACTIVE) -->
        <a class="flex items-center px-6 py-3 space-x-3 border-l-4 border-primary bg-primary-fixed text-primary font-bold transition-colors cursor-pointer active:opacity-80" href="<?php echo e(route('admin.events.index')); ?>">
            <span class="material-symbols-outlined">event</span>
            <span class="font-body-sm text-body-sm">Event Saya</span>
        </a>
        <!-- Scanner Tab -->
        <a class="flex items-center px-6 py-3 space-x-3 text-secondary hover:bg-surface-container-low transition-colors cursor-pointer active:opacity-80" href="<?php echo e(route('admin.scanner')); ?>">
            <span class="material-symbols-outlined">qr_code_scanner</span>
            <span class="font-body-sm text-body-sm">Scanner</span>
        </a>
        <!-- Keuangan Tab -->
        <a class="flex items-center px-6 py-3 space-x-3 text-secondary hover:bg-surface-container-low transition-colors cursor-pointer active:opacity-80" href="<?php echo e(route('admin.finance')); ?>">
...
<main class="md:ml-sidebar-width min-h-screen pt-16 md:pt-0 pb-20 md:pb-0">
    <div class="max-w-max-container mx-auto p-page-padding">
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
            ...
            <a href="<?php echo e(route('admin.events.create')); ?>" class="inline-flex items-center justify-center space-x-2 bg-primary-container text-on-primary-container px-6 py-2.5 rounded-lg hover:opacity-90 active:scale-95 transition-all shadow-none">
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
            <div class="col-span-1 md:col-span-2 relative">
                <form action="<?php echo e(route('admin.events.index')); ?>" method="GET" class="w-full">
                    <span class="material-symbols-outlined absolute left-3 top-1/2 -translate-y-1/2 text-outline">search</span>
                    <input name="search" value="<?php echo e(request('search')); ?>" class="w-full bg-surface-container-low border-[0.5px] border-outline-variant rounded-lg pl-10 pr-4 py-2 text-body-sm focus:border-primary-container focus:ring-0 transition-colors" placeholder="Cari nama event..." type="text"/>
                </form>
            </div>
            <div class="flex space-x-2 overflow-x-auto pb-1">
                <a href="<?php echo e(route('admin.events.index')); ?>" class="px-4 py-2 <?php echo e(!request('status') ? 'bg-primary text-on-primary' : 'bg-surface border-[0.5px] border-outline-variant text-secondary hover:bg-surface-container'); ?> rounded-lg text-label-md shrink-0 transition-colors">Semua</a>
                <a href="<?php echo e(route('admin.events.index', ['status' => 'active'])); ?>" class="px-4 py-2 <?php echo e(request('status') === 'active' ? 'bg-primary text-on-primary' : 'bg-surface border-[0.5px] border-outline-variant text-secondary hover:bg-surface-container'); ?> rounded-lg text-label-md shrink-0 transition-colors">Active</a>
                <a href="<?php echo e(route('admin.events.index', ['status' => 'ended'])); ?>" class="px-4 py-2 <?php echo e(request('status') === 'ended' ? 'bg-primary text-on-primary' : 'bg-surface border-[0.5px] border-outline-variant text-secondary hover:bg-surface-container'); ?> rounded-lg text-label-md shrink-0 transition-colors">Selesai</a>
            </div>
        </div>
            ...
            <table class="w-full text-left border-collapse min-w-[800px]">
                ...
                <tbody class="divide-y-[0.5px] divide-outline-variant">
                    ...
                    <tr class="<?php echo e($rowClass); ?>">
                        <td class="px-6 py-4">
                            <div class="w-12 h-16 rounded overflow-hidden bg-surface-container-high <?php echo e($isEnded ? 'grayscale' : ''); ?>">
                                <img src="<?php echo e($banner); ?>" class="w-full h-full object-cover" alt="Banner Event">
                            </div>
                        </td>
                        <td class="px-6 py-4">
                            <p class="font-body-sm text-body-sm font-bold text-on-surface"><?php echo e($event->title); ?></p>
                            <p class="text-caption text-secondary"><?php echo e($event->city ?? $event->location_type); ?></p>
                        </td>
                        <td class="px-6 py-4">
                            <span class="bg-surface-container-high px-2 py-1 rounded text-caption text-on-surface-variant"><?php echo e($event->category); ?></span>
                        </td>
                        <td class="px-6 py-4">
                            <p class="font-body-sm text-body-sm text-on-surface"><?php echo e(\Carbon\Carbon::parse($event->start_date)->format('d M Y')); ?></p>
                            <p class="text-caption text-secondary"><?php echo e(\Carbon\Carbon::createFromTimeString($event->start_time)->format('H:i')); ?> WIB</p>
                        </td>
                        <td class="px-6 py-4">
                        ...
                        <td class="px-6 py-4 text-right">
                            <div class="flex justify-end space-x-1">
                                <a href="<?php echo e(route('admin.events.show', $event->id_event)); ?>" class="p-2 text-primary hover:bg-primary-fixed rounded transition-colors" title="Detail">
                                    <span class="material-symbols-outlined text-[18px]">visibility</span>
                                </a>
                                <a href="<?php echo e(route('admin.events.edit', $event->id_event)); ?>" class="p-2 text-primary hover:bg-primary-fixed rounded transition-colors" title="Edit">
                                    <span class="material-symbols-outlined text-[18px]">edit</span>
                                </a>
                                <form action="<?php echo e(route('admin.events.destroy', $event->id_event)); ?>" method="POST" class="inline" onsubmit="return confirm('Hapus event ini? Tindakan ini tidak bisa dibatalkan.')">
        ...
        <div class="mt-6 flex flex-col sm:flex-row items-center justify-between border-t-[0.5px] border-outline-variant pt-4 gap-4">
            <span class="text-caption text-secondary">Menampilkan <?php echo e($events->firstItem()); ?>-<?php echo e($events->lastItem()); ?> dari <?php echo e($events->total()); ?> Event</span>
            <div class="flex space-x-2">
                ...
                <?php else: ?>
                    <a href="<?php echo e($events->previousPageUrl()); ?>" class="p-2 border-[0.5px] border-outline-variant rounded-lg hover:bg-surface-container transition-colors">
                        <span class="material-symbols-outlined">chevron_left</span>
                    </a>
                <?php endif; ?>
                ...
                <?php $__currentLoopData = $events->getUrlRange(1, $events->lastPage()); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $page => $url): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <?php if($page == $events->currentPage()): ?>
                        <button class="px-4 py-2 bg-primary-fixed text-primary font-bold rounded-lg text-label-md"><?php echo e($page); ?></button>
                    <?php else: ?>
                        <a href="<?php echo e($url); ?>" class="px-4 py-2 hover:bg-surface-container rounded-lg text-label-md transition-colors"><?php echo e($page); ?></a>
                    <?php endif; ?>
                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>

                <?php if($events->hasMorePages()): ?>
                    <a href="<?php echo e($events->nextPageUrl()); ?>" class="p-2 border-[0.5px] border-outline-variant rounded-lg hover:bg-surface-container transition-colors">
...
<nav class="fixed bottom-0 w-full z-50 md:hidden bg-surface border-t-[0.5px] border-outline-variant flex justify-around items-center h-16 pb-safe shadow-lg">
    <a class="flex flex-col items-center justify-center space-y-1 text-secondary" href="<?php echo e(route('admin.dashboard')); ?>">
        <span class="material-symbols-outlined">grid_view</span>
        <span class="font-label-md text-label-md">Dashboard</span>
    </a>
    <a class="flex flex-col items-center justify-center space-y-1 text-primary font-bold" href="<?php echo e(route('admin.events.index')); ?>">
        <span class="material-symbols-outlined">confirmation_number</span>
        <span class="font-label-md text-label-md">Events</span>
    </a>
    <a class="flex flex-col items-center justify-center space-y-1 text-secondary" href="<?php echo e(route('admin.scanner')); ?>">
        <span class="material-symbols-outlined">center_focus_weak</span>
        <span class="font-label-md text-label-md">Scan</span>
    </a>
    <a class="flex flex-col items-center justify-center space-y-1 text-secondary" href="<?php echo e(route('admin.finance')); ?>">
...
<?php /**PATH D:\laragon\www\JVC26\gatemate\resources\views/admin/events/index.blade.php ENDPATH**/ ?>