...
<main class="pt-16 pb-20">
    ...
    <section class="w-full relative aspect-video md:aspect-[21/9] lg:aspect-[3/1] bg-surface-variant overflow-hidden">
        ...
        ?>
        <img class="w-full h-full object-cover" alt="Banner <?php echo e($event->title); ?>" src="<?php echo e($bannerSrc); ?>">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-gap-default">
            ...
            <div class="lg:col-span-8 flex flex-col gap-gap-default">
                ...
                <div class="flex flex-col gap-3">
                    <div class="flex items-center gap-2">
                        <span class="bg-coral-light text-coral-dark px-3 py-1 rounded-[10px] font-label-md text-[11px] uppercase tracking-wider"><?php echo e($event->category); ?></span>
                    </div>
                    <h1 class="font-headline-lg-mobile md:font-headline-lg text-headline-lg-mobile md:text-headline-lg text-on-surface"><?php echo e($event->title); ?></h1>
                    <div class="flex flex-wrap items-center gap-6 text-on-surface-variant py-2">
                        <div class="flex items-center gap-2">
                            ...
                            <span class="font-body-md text-body-md"><?php echo e(\Carbon\Carbon::parse($event->start_date)->translatedFormat('d M Y')); ?></span>
                        </div>
                        <div class="flex items-center gap-2">
                            <span class="material-symbols-outlined text-[20px]" data-icon="location_on">location_on</span>
                            <span class="font-body-md text-body-md"><?php echo e($event->venue_name ?? $event->city ?? 'Lokasi Online'); ?></span>
                        </div>
                        <div class="flex items-center gap-2">
                            <span class="material-symbols-outlined text-[20px]" data-icon="schedule">schedule</span>
                            <span class="font-body-md text-body-md"><?php echo e(\Carbon\Carbon::createFromFormat('H:i:s', $event->start_time)->format('H:i')); ?> - Selesai WIB</span>
                        </div>
                    </div>
                    <p class="font-body-md text-on-surface-variant">Oleh: <?php echo e($event->admin->organization_name ?? $event->admin->full_name); ?></p>
                </div>
                ...
                <div class="bg-surface-container-lowest border border-divider card-radius p-6 flex flex-col gap-4">
                    ...
                    <div class="font-body-lg text-body-lg text-on-surface-variant leading-relaxed">
                        <p style="white-space: pre-wrap;"><?php echo e($event->description ?: 'Tidak ada deskripsi yang tersedia.'); ?></p>
                    </div>
                </div>
                ...
                <div class="flex flex-col gap-4">
                    ...
                    <?php if($event->maps_link): ?>
                        ...
                        <?php if($isEmbed): ?>
                            <div class="w-full h-64 bg-surface-f5 border border-divider rounded-[14px] overflow-hidden group relative">
                                ...
                                <a href="<?php echo e($event->maps_link); ?>" target="_blank" class="absolute bottom-4 left-4 bg-white/90 backdrop-blur-md px-4 py-2 card-radius border border-divider flex items-center gap-2 hover:bg-white transition-colors cursor-pointer" style="text-decoration:none;">
                        ...
                        <?php else: ?>
                            <div class="w-full h-64 bg-surface-f5 border border-divider rounded-[14px] overflow-hidden group relative cursor-pointer flex flex-col items-center justify-center text-on-surface-variant">
                                ...
                                <a href="<?php echo e($event->maps_link); ?>" target="_blank" class="mt-4 bg-white/90 backdrop-blur-md px-4 py-2 card-radius border border-divider flex items-center gap-2 hover:bg-white transition-colors" style="text-decoration:none;">
                                    <span class="material-symbols-outlined text-coral-red" data-icon="directions" data-weight="fill" style="font-variation-settings: 'FILL' 1;">directions</span>
                                    <span class="font-label-md text-label-md text-on-surface">Buka di Maps</span>
                                </a>
                            </div>
                    ...
                    <p class="text-on-surface-variant font-body-md text-body-md px-1"><?php echo e($event->location_details ?: 'Lokasi lengkap tidak tersedia.'); ?></p>
                </div>
            ...
            <div class="lg:col-span-4">
                <div class="sticky top-24 flex flex-col gap-gap-tight">
                    ...
                    <?php if($event->ticketTiers->isEmpty()): ?>
                    ...
                    <?php else: ?>
                            ...
                            <?php $__currentLoopData = $event->ticketTiers; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $tier): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <div class="bg-white border border-divider card-radius p-card-padding flex flex-col gap-3 transition-all hover:border-coral-red/30">
                                    <div class="flex justify-between items-start">
                                        <div>
                                            <h3 class="font-headline-sm text-headline-sm text-on-surface"><?php echo e($tier->tier_name); ?></h3>
                                            <p class="font-caption text-caption text-on-surface-variant">Akses event khusus</p>
                                        </div>
                                    </div>
                                    <div class="flex items-center justify-between mt-2">
                                        <span class="font-headline-md text-headline-md text-on-surface">Rp <?php echo e(number_format($tier->price, 0, ',', '.')); ?></span>
                                        <a href="javascript:void(0)" class="bg-coral-red text-white px-6 py-2 pill-radius font-label-md text-label-md hover:bg-opacity-90 active:scale-95 transition-all text-center" 
                                           onclick="document.getElementById('checkout-modal-<?php echo e($tier->id_tier ?? $tier->id); ?>').classList.remove('hidden'); return false;" style="text-decoration:none;">Beli Tiket</a>
                                    </div>
                                
                                <!-- Modal Popup Checkout untuk Tier ini -->
                                <div id="checkout-modal-<?php echo e($tier->id_tier ?? $tier->id); ?>" class="fixed inset-0 z-50 flex items-center justify-center p-4 hidden">
                                    ...
                                    <div class="relative bg-white border border-outline-variant rounded-[14px] shadow-2xl w-full max-w-[520px] overflow-hidden flex flex-col animate-in fade-in zoom-in duration-300">
                                        ...
                                        <form action="<?php echo e(route('checkout.process')); ?>" method="POST" onsubmit="processCheckout(event, this)">
                                            <?php echo csrf_field(); ?>
                                            <input type="hidden" name="event_id" value="<?php echo e($event->id_event ?? $event->id); ?>">
                                            <input type="hidden" name="tier_id" value="<?php echo e($tier->id_tier ?? $tier->id); ?>">
                                            ...
                                            <div class="p-6 space-y-6 overflow-y-auto max-h-[60vh]">
                                                ...
                                                <div class="bg-surface-container-low border border-outline-variant p-4 rounded-xl flex justify-between items-center">
                                                    <div>
                                                        ...
                                                        <h4 class="font-headline-sm text-headline-sm text-on-surface"><?php echo e($tier->tier_name); ?></h4>
                                                    </div>
                                                    <p class="font-headline-sm text-headline-sm text-primary font-bold">Rp <?php echo e(number_format($tier->price, 0, ',', '.')); ?></p>
                                                </div>
                                                
                                                <!-- Additional Questions Section -->
                                                <?php if($event->customQuestions && $event->customQuestions->count() > 0): ?>
                                                    <div class="space-y-4">
                                                        <h3 class="font-headline-sm text-headline-sm text-on-surface font-bold">Informasi Tambahan</h3>
                                                        <?php $__currentLoopData = $event->customQuestions; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $question): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                                            <div class="space-y-2">
                                                                <label class="font-label-md text-label-md text-on-surface-variant"><?php echo e($question->question_text); ?></label>
                                                                <textarea name="answers[<?php echo e($question->id_question); ?>]" class="w-full bg-surface-f5 border border-divider rounded-[10px] p-3 text-body-md font-body-md focus:ring-1 focus:ring-coral-red focus:border-coral-red outline-none transition-all resize-none h-24" placeholder="Tuliskan jawaban Anda di sini..." required></textarea>
                                                            </div>
                                                ...
                                                <div class="pt-4 border-t border-outline-variant">
                                                    <div class="flex justify-between items-center mb-1">
                                                        ...
                                                        <span class="text-headline-md font-headline-md text-primary font-extrabold">Rp <?php echo e(number_format($tier->price, 0, ',', '.')); ?></span>
                                                    </div>
                        ...
                        <?php else: ?>
                            <div class="bg-white border border-divider card-radius p-card-padding flex flex-col gap-3 items-center text-center py-8">
                                ...
                                <a href="<?php echo e(route('signin')); ?>" class="mt-2 bg-coral-red text-white px-6 py-2 pill-radius font-label-md text-label-md hover:bg-opacity-90 active:scale-95 transition-all" style="text-decoration:none;">Masuk Sekarang</a>
                            </div>
                        <?php endif; ?>
                    <?php endif; ?>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\laragon\www\JVC26\gatemate\resources\views/events/show.blade.php ENDPATH**/ ?>